...
def deletion():
    ...
    class PageThree3(Frame):
        def __init__(self, parent, controller):
            ...
            page_two = Button(self, text="Default", font='Times 17 bold', padx=30, pady=10, command=deletion2.deletion2)
    ...
    class byuser(Frame):
            def __init__(self, parent, controller):
                ...
                def showform():
                    for widget in self.winfo_children():
                        widget.destroy()
                    label = Label(self, text="Enter the number you wanted to delete in the array:", bg='black', fg='white', font='Georgia 24 bold', pady=30)
                    label.grid(row=0, column=0)
                    entr = Entry(self, width=5, font=('Times', 28))
                    entr.grid(row=1, column=0, padx=5, pady=50)
                    def input3():
                        ...
                        d = (int(entr.get()))
                        print(d)
                    btn = Button(self, text="SUBMIT", font='Times 17 bold', padx=30, pady=10,command=lambda: [input3(), deletion2.deletion2()])
                    btn.grid(row=2, columnspan=5, pady=70)
